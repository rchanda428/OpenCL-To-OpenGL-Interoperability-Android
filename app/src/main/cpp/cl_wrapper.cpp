...
//--------------------------------------------------------------------------------------
#include "../jniIncludes/cl_wrapper.h"
#include "../jniIncludes/util.h"
...
#define LOG_TAG    "cl_wrapper.cpp"

#define DPRINTF1(...)  __android_log_print(ANDROID_LOG_DEBUG,LOG_TAG,__VA_ARGS__)
...

{
    ...
    m_context = clCreateContext(context_properties, 1, &m_device, NULL, NULL, &err);
    if (err != CL_SUCCESS)
    {
...
{
    cl_int err = 0;
    cl_program program = clCreateProgramWithSource(m_context, source_len, source, NULL, &err);
    if (err != CL_SUCCESS)
    {
